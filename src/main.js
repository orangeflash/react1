...
class Sider extends React.Component {
    ...
    componentWillMount() {
        ...
        const keyArray = ['home', 'about', 'skill', 'project', 'contact']/*侧边栏的key*/
        let num = 0
        keyArray.forEach((key, index) => {/*map是执行一个操作返回新数组，不改变原数组，forEach是对数组执行一个操作不返回*/
            if (newkey == key) {
                num = index
            }
        })
        this.setState({
            current: newkey,/*控制左侧边栏和右边内容对应*/
            num,
        })
    }
            ...
            <div className={collapse ? "layout-aside layout-aside-collapse" : "layout-aside"}>
                <aside className="layout-sider">
                    <div className="layout-logo">
                        ...
                        {!collapse && <span className="title-text">刘航</span>}
                    </div>
                    <Menu mode="inline"
                        theme="dark"
                        defaultSelectedKeys={['home']}
                        selectedKeys={[this.state.current]}/*控制侧边栏哪个选项被选中*/
